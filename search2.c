                ...
                for (int pos1 = lowRange; pos1 <= hiRange; pos1 = pos1 + 1)/*Inner Loop3*/ {
                    ...
                    *md_value_hex2 = 0;
                    ...
                    for (j = 0; j < md_len2; j++) {
                        ...
                        sprintf(hex_string, "%02x", md_value2[j]);//Converting ascii string to hex string
                        // printf("%s", hex_string);
...
//Step 5 Finding another 4-byte array that hashes to the same value as "harD"
 
//Declaring char and int variables
	//An unsigned char is a (unsigned) byte value (0 to 255)
    unsigned char md_value_hex3[EVP_MAX_MD_SIZE] = "";
    unsigned char md_value_computed3[EVP_MAX_MD_SIZE];
    unsigned char md_value3[EVP_MAX_MD_SIZE];
    unsigned int md_len3, b;
            ...
            for (int pos2 = lowRange; pos2 <= hiRange; pos2 = pos2 + 1) /*Inner Loop2*/{
                for (int pos1 = lowRange; pos1 <= hiRange; pos1 = pos1 + 1)/*Inner Loop3*/ {
                    ...
                    *md_value_hex3 = 0;
                    ...
                    EVP_DigestInit_ex(mdctx, md, NULL);/*Set up digest context*/
                    // EVP_DigestUpdate(mdctx, charString, strlen(charString));
                    EVP_DigestUpdate(mdctx, charString3, 4);/*Hashes cnt bytes of data at d into the digest context ctx*/
                    EVP_DigestFinal_ex(mdctx, md_value3, &md_len3);/*Retrieves the digest value from ctx and places it in md*/
                    EVP_MD_CTX_free(mdctx);/*Cleans up digest context ctx and frees up the space allocated to it*/
                    ...
                    for (i = 0; i < md_len3; i++) {
						//Instead of printing on console, 
						//it stores output on char buffer which are specified in sprintf.
                        sprintf(hex_string, "%02x", md_value3[i]);//Converting ascii string to hex string
                        // printf("%s", hex_string);
						//Concetenate strings by appending a copy of the source string [hex_string]
						//to the destination string md_value_hex.
                        strcat(md_value_hex3, hex_string);//
                        // printf("%x", md_value[i]);
                    } 
                    // printf("\n");
					...
					//Comparing the computed md5 hash value with the given one
                    if (strcmp(charString2, md_value_hex3) == 0) {
                        printf("Given: %s == Computed: %s\n", charString2, md_value_hex3);
                        printf("**********MATCH!**********\n");
						//This is the charString generated in the Outer/Inner Loops
                        printf("charString: %c%c%c%c\n", charString3[0], charString3[1], charString3[2], charString3[3]);
             //To be sure a different 4-byte string is found, we compare the 4-byte string to "harD" if it is the same, continue 
             //running, if it isnt the same, exit.
